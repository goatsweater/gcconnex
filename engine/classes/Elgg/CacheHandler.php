...
class CacheHandler {
	...
	protected function setupSimplecache() {
		...
		$dblink = pdo_connect($this->config->dbhost, $this->config->dbuser, $this->config->dbpass, true);
		if (!$dblink) {
			$this->send403('Cache error: unable to connect to database server');
		}

		if (!pdo_select_db($this->config->dbname, $dblink)) {
		...
		$result = pdo_query($query, $dblink);
		if ($result) {
			while ($row = pdo_fetch_object($result)) {
