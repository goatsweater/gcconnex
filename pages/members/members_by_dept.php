...
foreach ($department_listing as $email => $department_name) {
	if (is_string($department_name)) {
		...
		$department_name_arr = explode('|',$department_name);
		if ($department_name_arr[1] !== $get_department)
...
if ($sort_type === 'alpha')
{
	asort($selected_department);	// sort by value
} else {
	ksort($selected_department);	// sort by key
}
